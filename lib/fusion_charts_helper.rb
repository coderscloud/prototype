...
module FusionChartsHelper
  ...
  def render_chart(chart_swf,str_url,str_data,chart_id,chart_width,chart_height,debug_mode=false,register_with_js=false,options={})
    ...
    safe_concat("\t\t<!-- START Script Block for Chart "+chart_id+" -->\n\t\t")
    safe_concat(content_tag(:div,"\n\t\t\t\tChart.\n\t\t",{:id=>chart_id+"Div",:align=>"center"})) 
    safe_concat("\n\t\t<script type='text/javascript'>\n")
    safe_concat("\t\t\t\tvar chart_"+chart_id+"=new FusionCharts("+fcParamsInJson+").render();\n")
    safe_concat("\t\t</script>\n")
    safe_concat("\t\t<!-- END Script Block for Chart "+chart_id+" -->\n")
  ...
  def render_chart_html(chart_swf,str_url,str_data,chart_id,chart_width,chart_height,debug_mode=false,register_with_js=false,options={})
    ...
    safe_concat("\t\t<!-- START Code Block for Chart "+chart_id+" -->\n\t\t")
    #safe_concat(content_tag(:object,param_tags,object_attributes))
    safe_concat(content_tag(:object, object_attributes) do safe_concat(param_tags)end)  
  ...
  def get_UTF8_BOM
    ...
    return utf8_str.force_encoding("UTF-8")
  end
  
  def get_monthes(d1,d2)

    eom = d1.end_of_month #=> Sat, 30 Jun 2007
    bom = d1.beginning_of_month
    month = l bom, :format=> "%B"
    columns= [{:start=>bom, :end=>eom, :month=>month}]
   # mth_ends = [eom] #=> [Sat, 30 Jun 2007]
    while eom < d2
      eom = eom.advance(:days => 1).end_of_month
      bom = eom.beginning_of_month
      month = I18n.l bom, :format=> "%B"
      columns << {:start=>bom, :end=>eom, :month=>month}
    end
    @monthes_list=columns
