...
Please note:
The latest version has changed the modal setting for the edit/add forms to be false rather than true. The reason
for this is when modal is true then it causes all sorts of problems with datepicker when the datepicker field
is the first field. If this is not desirable for you fork the project and change as required.
